...
let Like = ({ resultId, likeCount, isLike }: IdPropType) => {
    ...
    <div className='pt-4'>
      <div className='flex justify-center align-center'>
        {like ? (
          <div
            onClick={() => {
              fetch(`/api/like/postLike`, { method: 'POST', body: JSON.stringify({ id: resultId, likeCount: likeCount }) }).then((response) => {
                if (response.ok) {
                  console.log(1);
                }
              });
      ...
      <div className=' flex justify-center align-center text-lg font-semibold'>{count}</div>
    </div>
  );
};
